  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <svg width="180" height="100" viewBox="0 0 180 100">
        {/* Track */}
        <path d="M 20 90 A 70 70 0 0 1 160 90" fill="none" stroke="var(--border)" strokeWidth="8" strokeLinecap="round" />
        {/* Fill */}
        <path d="M 20 90 A 70 70 0 0 1 160 90" fill="none" stroke={color} strokeWidth="8" strokeLinecap="round"
          strokeDasharray={`${(score / 100) * 220} 220`} opacity="0.3" />
        {/* Needle */}
        <g transform={`rotate(${angle}, 90, 90)`}>
          <line x1="90" y1="90" x2="90" y2="28" stroke={color} strokeWidth="2.5" strokeLinecap="round" />
          <circle cx="90" cy="90" r="5" fill={color} />
        </g>
        {/* Score */}
        <text x="90" y="78" textAnchor="middle" fill={color}
          style={{ fontFamily: 'Space Mono, monospace', fontSize: 22, fontWeight: 700 }}>{score}</text>
      </svg>
      <span style={{
        fontFamily: 'var(--font-mono)', fontSize: 11,
        color, background: `${color}18`,
        border: `1px solid ${color}40`,
        borderRadius: 4, padding: '3px 10px', letterSpacing: '0.1em',
      }}>{label} RISK</span>
    </div>
  )
}

function SignalRow({ label, score, detail }) {
  const color = score >= 75 ? 'var(--risk-critical)'
    : score >= 50 ? 'var(--risk-high)'
    : score >= 25 ? 'var(--risk-medium)'
    : 'var(--risk-low)'
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
      <div style={{ width: 90, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-muted)', letterSpacing: '0.06em' }}>{label}</div>
      <div style={{ flex: 1, height: 3, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ width: `${score}%`, height: '100%', background: color, borderRadius: 2, boxShadow: `0 0 6px ${color}` }} />
      </div>
      <div style={{ width: 36, fontFamily: 'var(--font-mono)', fontSize: 11, color, textAlign: 'right' }}>{score}</div>
    </div>
  )
}
...
function InsightCard({ insight }) {
  ...
  const icons = { blocker: '⛔', risk: '⚠', decision: '◆', sentiment: '◎', dependency: '⇢' }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        <span style={{ fontSize: 12 }}>{icons[insight.insight_type] || '•'}</span>
      ...
      <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{insight.content}</p>
    </div>
  )
}
    ...
    <div style={{ padding: 32 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 800 }}>{team?.name}</h1>
          {team?.active_sprint_name && (
            <div style={{ marginTop: 4, fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--green)' }}>
              ● {team.active_sprint_name}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => navigate(`/chat/${teamId}`)} style={{
            ...
            padding: '9px 16px', color: '#000', cursor: 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={{
          ...
          <div style={{ marginTop: 12, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-muted)', textAlign: 'center' }}>
        ...
        <div style={{
          ...
          )) : <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>No data yet — run a sync</div>}
        </div>
        ...
        <div style={{
          ...
          <ResponsiveContainer width="100%" height={160}>
            <RadarChart data={radarData}>
              ...
              <PolarAngleAxis dataKey="signal" tick={{ fill: 'var(--text-muted)', fontSize: 9, fontFamily: 'Space Mono' }} />
        ...
        <div style={{
          ...
          <ResponsiveContainer width="100%" height={120}>
            <LineChart data={trendData}>
              <XAxis dataKey="t" tick={{ fill: 'var(--text-muted)', fontSize: 9, fontFamily: 'Space Mono' }} axisLine={false} tickLine={false} />
              <YAxis domain={[0, 100]} tick={{ fill: 'var(--text-muted)', fontSize: 9, fontFamily: 'Space Mono' }} axisLine={false} tickLine={false} width={28} />
              <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'Space Mono', fontSize: 11 }} />
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {insights.length === 0 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: 12, fontFamily: 'var(--font-mono)' }}>NO INSIGHTS YET</div>
